...
var (
	...
	pipelinePluginUrl = flag.String("pipeline-plugin-url", utils.LookupEnvOrString("PIPELINE_PLUGIN_URL", "http://127.0.0.1:8081/api/v1/plugin"), "pipeline plugin url.")
	agentVersion      = flag.String("agent-version", utils.LookupEnvOrString("AGENT_VERSION", "latest"), "kubespace agent version.")
	agentRepository   = flag.String("agent-repository", utils.LookupEnvOrString("AGENT_REPOSITORY", "kubespace/agent"), "kubespace agent version.")
)
...
func main() {
	...
	var err error
	conf.AppConfig.PipelinePluginUrl = *pipelinePluginUrl
	conf.AppConfig.AgentVersion = *agentVersion
	conf.AppConfig.AgentRepository = *agentRepository
	svr, err := buildServer()
	if err != nil {
		panic(err)
	}
