...
import (
	...
	serializers2 "github.com/kubespace/kubespace/pkg/server/views/serializers"
...
func (clu *Cluster) create(c *Context) *utils.Response {
	var ser serializers2.ClusterCreateSerializers
	resp := &utils.Response{Code: code.Success}
...
func (clu *Cluster) apply(c *Context) *utils.Response {
	var ser serializers2.ApplyYamlSerializers
	if err := c.ShouldBind(&ser); err != nil {
...
func (clu *Cluster) delete(c *Context) *utils.Response {
	var ser []serializers2.DeleteClusterSerializers
	if err := c.ShouldBind(&ser); err != nil {
		klog.Errorf("bind params error: %s", err.Error())
		return &utils.Response{Code: code.ParamsError, Msg: err.Error()}
	}
...
func (clu *Cluster) resourceSSE(c *Context) *utils.Response {
	...
	var ser serializers2.ClusterSSESerializers
