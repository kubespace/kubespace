...
func (p *PipelineTriggerController) triggerHandle(obj interface{}) error {
	...
	} else if trigger.Type == types.PipelineTriggerTypeCron {
		return p.cronTrigger(pipelineWorkspace, pipeline, &trigger)
	}
	return nil
}
...
func (p *PipelineTriggerController) triggerCodePipeline(
	...
	if updated || trigger.NextTriggerTime != nil {
		// 更新triggerConfig到数据库，下次触发时间修改为空，等到再次代码更新时触发
...
func (p *PipelineTriggerController) cronTrigger(
	...
	return p.models.PipelineTriggerManager.Update(trigger.ID, &types.PipelineTrigger{
		UpdateTime:      time.Now(),
		NextTriggerTime: &sql.NullTime{Time: nextTriggerTime, Valid: true},
	})
}
...
func (p *PipelineTriggerController) cronTriggerCustomPipeline(
	...
	for _, source := range pipeline.Sources {
		...
		sourcePipelineWorkspace, err := p.models.PipelineWorkspaceManager.Get(sourcePipeline.WorkspaceId)
		if err != nil {
			return err
		}
		buildSources = append(buildSources, &types.PipelineBuildCustomSource{
			WorkspaceId:   sourcePipeline.WorkspaceId,
			WorkspaceName: sourcePipelineWorkspace.Name,
			PipelineId:    sourcePipeline.ID,
			PipelineName:  sourcePipeline.Name,
			BuildId:       sourceBuilds[0].ID,
			BuildNumber:   sourceBuilds[0].BuildNumber,
			BuildOperator: sourceBuilds[0].Operator,
			IsBuild:       true,
		})
	}
	if err := p.models.PipelineTriggerEventManager.Create(&types.PipelineTriggerEvent{
		...
		TriggerId:   trigger.ID,
		Status:      types.PipelineTriggerEventStatusNew,
		EventConfig: types.PipelineBuildConfig{CustomSources: buildSources},
		TriggerUser: trigger.UpdateUser,
		CreateTime:  time.Now(),
		UpdateTime:  time.Now(),
	}); err != nil {
		klog.Errorf("create pipeline trigger event error: %s", err.Error())
