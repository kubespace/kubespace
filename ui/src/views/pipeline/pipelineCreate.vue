      ...
      <div style="padding: 10px 8px 0px;">
        ...
        <el-form label-position="left" class="pipeline-form-item" label-width="80px">
          <el-form-item label="名称">
            <el-input v-model="createPipeline.name" autocomplete="off" placeholder="请输入流水线名称" size="small"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <div style="padding: 10px 8px 0px;" >
        ...
        <div class="stage-job-outer">
          ...
          <div v-for="(stage, i) in createPipeline.stages" :key="i">
            <div class="stage-job-line">
              <div class="stage-job-block">
                <div class="stage-job-block__stage" @click="openEditStageDialog(stage); dialogVisible=true;">
                  <span>#{{ i + 1 }} {{ stage.name }}</span>
                </div>
              </div>
              <div>
                <div class="stage-job-line__inner"></div>
                <a class="stage-job-line__circle" @click="openEditJobDialog(stage, 0); dialogVisible=true;"></a>
                <div class="stage-job-line__inner"></div>
              ...
              <div class="stage-job-block">
                <template  v-if="stage.jobs && stage.jobs.length > 1" >
                  <div v-for="(job, i) in stage.jobs" :key="i">
                    <template v-if="i >0">
                      <div class="stage-job-block__job">
                        <div class="stage-job-block__job-circle">
                          <div class="stage-job-line__circle" @click="openEditJobDialog(stage, i); dialogVisible=true;"></div>
                        </div>
                      </div>
...
import { createPipeline } from '@/api/pipeline/pipeline'
...
export default {
  ...
  methods: {
    ...
    savePipeline() {
      ...
      this.loading = true
      createPipeline(this.createPipeline).then((response) => {
    ...
    dialogSave() {
      if (this.dialogType == 'edit_stage') {
        this.dialogOriginData.name = this.dialogData.name
        this.dialogOriginData.trigger_mode = this.dialogData.trigger_mode
        ...
        this.createPipeline.stages.splice(this.dialogOriginData, 0, newStage)
      } else if(this.dialogType == 'add_job') {
        this.dialogOriginData.jobs.push(this.dialogData)
      }
      console.log(this.dialogOriginData)
