    ...
    <div class="dashboard-container" v-if="!editYaml">
      <el-form :model="dsInfo" :rules="dsRules" ref="dsInfoForm" label-width="120px"
        ...
        <el-form-item label="名称" prop="name">
          <el-input v-model="dsInfo.name" class="input-class" placeholder="Deployment名称"></el-input>
        </el-form-item>
                    ...
                    <el-row style="padding-top: 10px;" v-for="(item, idx) in vol[vol.type].items" :key="idx">
                      ...
                      <el-col :span="3">
                        <el-button plain size="small" style="padding-left: 10px; padding-right: 10px;" 
                          @click="vol['configMap'].items.splice(idx, 1)" icon="el-icon-minus"></el-button>
                      </el-col>
                    </el-row>
  ...
  methods: {
    ...
    create() {
      ...
      createYaml(cluster, yamlStr).then((resp) => {
        ...
        this.$router.push({name: 'deploymentDetail', params: {namespace: this.dsInfo.namespace, deploymentName: this.dsInfo.name}})
      }).catch(() => {
        // console.log(e) 
    ...
    getSpecContainers() {
      ...
      if(this.containers && this.containers.length > 0) {
        ...
        for(var c of this.containers) {
          ...
          if(c.ports && c.ports.length > 0) container.ports = c.ports
